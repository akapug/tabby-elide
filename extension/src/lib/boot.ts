...
async function cacheAtomic(obj: any) {
  ...
  await chrome.storage.local.set({ etag: obj.etag, version: obj.version })
}
...
async function boot(opts: { headless: boolean }) {
  ...
  const etag = (await chrome.storage.local.get('etag'))?.etag
  ...
  if (!ok) {
    ...
    return { from: 'cache', payload: cached }
  }
  let payload: any
